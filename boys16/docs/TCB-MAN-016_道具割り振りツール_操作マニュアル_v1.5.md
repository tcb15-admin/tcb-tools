...
| **第2部 詳細操作** | 慣れてから | 持ち帰り調整、履歴、手直し、班跨ぎ手動、トラブル時の対処 |
...
3. 画面上部に **STEP 1｜2｜3** と、右上に **マスタ・履歴・v1.6.3** が表示されればOKです。
...
直前に保存した割り振り結果を確認できます。**「この結果を元に持ち帰り調整」** で、前回の割り振りを基準に欠席・お茶当番だけ直せます（第2部）。STEP3 の **「元に再編集」** からも同じモードを開始できます。
...
## 2-1. 持ち帰り調整（前回の結果を元に直す）

欠席・お茶当番だけ変えたいときに使います。**持ち帰り調整モード**では、前回の割り振り（担当 map）を基準に、変わった部分だけ自動で振り直します。固定担当は維持されます。

### 開始方法（いずれか）

| 経路 | 操作 |
|------|------|
| 方法A | STEP1 **「◀ 前回」** → **「この結果を元に持ち帰り調整」** |
| 方法B | STEP3 **「元に再編集」**（いま表示中の結果を基準） |
| 方法C | **履歴** → 活動日をタップ → 詳細の **「この結果を元に持ち帰り調整」** |

### モード中の画面

- STEP1・STEP2 に **青い「持ち帰り調整モード」バナー** が表示されます。
- **活動日は本日** に設定されます（当日の欠席・お茶当番を直す想定）。
...
1. 上記のいずれかで持ち帰り調整を開始。
2. STEP2 で欠席・お茶当番・グループを直す。
3. **「割振りを実行」** → STEP3 で結果を確認。
4. 必要ならプルダウンで手直し → 印刷/PDF → **実施確定**。
...
| 別のスマホで同じ結果を見たい | マスタ・履歴はクラウド同期。共有はPDFが確実。履歴から**持ち帰り調整**を別端末で開始することも可能 |
...
```
優先度 ＝ 過去担当回数（PAST）× 3 ＋ 当日の負荷スコア × 1 ＋ 当日の担当件数 × 6
```

- **PAST（過去担当回数）** … 「実施確定」を押した活動だけカウント。**割振りを実行しただけでは増えません。**
- **当日の負荷スコア** … その日すでに持つ道具の重さ・長さを合計したイメージ。
- **当日の担当件数** … その日すでに何個振られたか（多い人は後回しにしやすい）。
...
欠席・お茶当番だけ変えたいときは、**前回の割り振り map を基準**に、変わったメンバーに関係する道具だけをプールから再配分します。固定担当は維持。活動パターンが前回と違っても map ベースで調整できます（STEP2 で割振りを実行し直す）。活動日は調整開始時に**本日**に設定されます。前当番が基準にいない道具は引き継がれないことがあり、STEP3 で手動割当します。
...
**文書末尾** — 版 1.6（2026-05-22、ツール v1.6.3）。仕様の詳細はリポジトリ内 `boys15/docs/TCB-SPEC-001` 等を参照。本書は16期道具マネージャー向けの操作手順に特化しています。
