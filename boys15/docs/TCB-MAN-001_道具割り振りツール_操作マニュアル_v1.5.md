...
| 対象 | 15期 道具マネージャー |
| 作成日 | 2026年5月 |
| 最終更新 | 2026年7月5日（ツール v1.7.2・利用シーン別フロー追加） |
...
3. 画面上部に **STEP 1｜2｜3** と、右上に **マスタ・履歴・v1.7.2** が表示されればOKです。
...
flowchart TD
    S([開始]) --> L[ログイン]
    L --> C{前回確定データあり?}
    C -->|いいえ| Z[STEP1 片方試合を選び<br/>1-3〜1-5 の通常手順]
    Z --> ZE([終了])
    C -->|はい| P1[STEP1「前回の結果を元に割振る」]
    P1 --> B[青バナー「前回の結果を元に調整」]
    B --> S1[STEP1：片方試合・活動日・メンバー未発表]
    S1 --> S2[STEP2：欠席・お茶当番・試合組]
    S2 --> RUN[「割振りを実行」]
    RUN --> S3[STEP3：結果確認]
    S3 --> U{未割当あり?}
    U -->|はい| M[手動割当<br/>班跨ぎチェック可]
    M --> S3
    U -->|いいえ| H[入れ替え案内・班跨ぎ特記を確認]
    H --> SH[展開情報 → LINE／PDF]
    SH --> CF[実施確定 PAST反映]
    CF --> E([終了])
```
...
```mermaid
flowchart TD
    S([開始]) --> P3[STEP3 まで到達済み]
    P3 --> MU{メンバー未発表 ON?}
    MU -->|OFF| N[組別表示で共有<br/>通常の展開]
    N --> E([終了])
    MU -->|ON| OP[展開情報を開く]
    OP --> OPT[送る内容：LINE本文 ✓ PDF ✓]
    OPT --> NOTE[保護者確認URL は<br/>実施確定後に有効 → フロー6]
    NOTE --> SEND[LINEへ展開 or PDFを保存]
    SEND --> CF[実施確定]
    CF --> E
```

> **注意** … 組名・班列は PDF に出ません。道具の種類から推測される可能性は残ります（注意事項 1 参照）。
...
```mermaid
flowchart TD
    S([開始]) --> ST[前回ベース調整を開始<br/>フロー1と同じ入口]
    ST --> AH{記録と実保有がズレている?}
    AH -->|いいえ| S2[STEP2：今日の欠席・当番・試合組]
    AH -->|はい| FIX[青バナー「現地の実保有に補正」]
    FIX --> EDIT[変わった道具だけプルダウン修正]
    EDIT --> SAVE[「この内容で補正する」]
    SAVE --> S2
    S2 --> RUN[「割振りを実行」]
    RUN --> S3[STEP3：入れ替え案内確認]
    S3 --> X{班跨ぎ特記あり?}
    X -->|はい| READ[橙色の受け渡し案内を確認]
    X -->|いいえ| SH
    READ --> SH[展開情報 → 実施確定]
    SH --> E([終了])
```
...
flowchart TD
    S([開始]) --> DONE[すでに展開済み]
    DONE --> EDIT[STEP3：プルダウンで手直し<br/>Undo／Redo 可]
    EDIT --> U{未割当残った?}
    U -->|はい| M[手動割当 or STEP2 見直し]
    M --> EDIT
    U -->|いいえ| RESH[展開情報を再度開く]
    RESH --> RESEND[更新内容を LINE／PDF で再送]
    RESEND --> CF[「実施確定」をもう一度]
    CF --> NOTE[同一活動日の PAST が差し替え]
    NOTE --> E([終了])
```
...
flowchart TD
    S([開始]) --> PAT{活動パターン}
    PAT -->|両方練習＋場所分離| R1[STEP1：両方練習<br/>「練習場所が分かれる」ON]
    R1 --> R2[STEP2：欠席・当番<br/>マスタ会場列で室内A／屋外B]
    PAT -->|両方試合| Y1[STEP1：両方試合]
    Y1 --> Y2[STEP2：欠席・当番<br/>A/B 試合組の振り分け]
    R2 --> RUN[割振り実行]
    Y2 --> RUN
    RUN --> S3[STEP3 → 展開 → 実施確定]
    S3 --> MERGE[必要ならフロー1・2へ]
    MERGE --> E([終了])
```
...
```mermaid
flowchart TD
    S([開始]) --> BASE[フロー1等で STEP3 まで完了]
    BASE --> FIRST[展開情報で初回共有 LINE／PDF]
    FIRST --> CF[実施確定 ※URL発行の前提]
    CF --> OP2[展開情報を再度開く]
    OP2 --> URL[保護者確認URL ✓ を選択]
    URL --> LINE[LINEへ展開]
    LINE --> PV[保護者：閲覧専用ページで確認]
    PV --> REP{交代報告あり?}
    REP -->|いいえ| E([終了])
    REP -->|はい| MGR[ヘッダ「報告」→ 承認 or 却下]
    MGR --> NEED{内容反映が必要?}
    NEED -->|はい| F4[フロー4：手直し・再確定]
    NEED -->|いいえ| E
    F4 --> E
    OP2 --> STOP[必要なら「公開を停止」]
```

> **覚えておく** … 保護者確認 URL は **実施確定済みの直近2日分** が対象です。確定前は URL にチェックを入れられません。
...
**文書末尾** — 版 1.10（2026-07-05、ツール v1.7.2・利用シーン別フロー追加）。仕様の詳細はリポジトリ内 `boys15/docs/TCB-SPEC-001` 等を参照。本書は15期道具マネージャー向けの操作手順に特化しています。
